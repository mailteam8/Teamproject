    ...
    try:
        ...
        for r in readings:
            ...
            predictions.append({
                ...
                "fall_detected": r.get("fall_detected"),
                "sos_triggered": r.get("sos_triggered"),
                "geo_location": r.get("geo_location"),
                "prediction": result
            })

        # التشخيص والتوصية (شامل)
        if any(r.get("sos_triggered") for r in readings):
            diagnosis = "SOS triggered"
            recommendation = "تم إطلاق استغاثة تلقائية، يجب التدخل فورًا."
        elif any(r.get("fall_detected") for r in readings):
            diagnosis = "Fall detected"
            recommendation = "تم رصد سقوط المريض، يجب التدخل فورًا."
        ...
        elif min([r.get("activity_level", 0) for r in readings]) < 100:
            diagnosis = "Low activity risk"
            recommendation = "النشاط البدني منخفض جدًا، يُفضل تشجيع المريض على الحركة لتجنب المضاعفات."
        elif max([r.get("activity_level", 0) for r in readings]) > 10000 and pulse_abnormal > 0:
            diagnosis = "Overexertion risk"
            recommendation = "النشاط البدني مرتفع جدًا مع نبض غير طبيعي، يجب تقليل الجهد ومراجعة الطبيب."
        else:
...
# ✅ التقرير دائمًا
        report_data = {
            ...
            "pat_id": pat_id,
            "geo_location": readings[0].get("geo_location")  # إضافة الموقع للتقرير
        }
        report_response = supabase.table("tbl_report").insert(report_data).execute()

        # ✅ التنبيه عند الحالات الخطيرة أو السقوط أو الاستغاثة أو النشاط غير الطبيعي
        alert_response = None
        if diagnosis in ["Respiratory risk", "Cardiac stress", "Critical condition", 
                         "Unstable angina", "Hypertension crisis", "Sepsis suspected", 
                         "Hypothermia risk", "Fall detected", "SOS triggered", "Overexertion risk"]:
            alert_data = {
                ...
                "is_seen": False,
                "pat_id": pat_id,
                "geo_location": readings[0].get("geo_location")  # إضافة الموقع للتنبيه
            }
